// api/transcribe.js - Episode title search + Whisper transcription with heartbeat
...
const CONFIG = {
 ...
 HEARTBEAT: {
   ...
   MESSAGES: [
     {
       message: "🎧 Deep-diving into your episode's semantic goldmine...",
       dad_joke: "Why don't podcasters ever get tired? Because they're always re-charging their episodes! 🔋",
       progress: 30
     },
     {
       message: "🔍 Extracting those niche keywords that'll connect you with your perfect audience...",
       dad_joke: "What do you call a podcast about gardening? A pod-cast! Get it? Pod... cast... 🌱",
       progress: 60
     },
     {
       message: "🚀 Almost ready to turn your episode into marketing magic...",
       dad_joke: "Why did the podcast go to therapy? It had too many issues to work through! 📻",
       progress: 85
     }
   ]
 }
...
/**
* Sends heartbeat updates to keep GPT connection alive
*/
function sendHeartbeat(res, messageIndex, episodeTitle) {
 const heartbeatData = CONFIG.HEARTBEAT.MESSAGES[messageIndex];
 if (!heartbeatData || res.finished) return;
 
 const update = {
   status: 'processing',
   progress: heartbeatData.progress,
   message: heartbeatData.message,
   dad_joke: heartbeatData.dad_joke,
   episode_title: episodeTitle,
   note: "Quality analysis takes time - we're extracting every growth opportunity! ⏰"
 };
 
 try {
   res.write(JSON.stringify(update) + '\n');
   console.log(`Heartbeat ${messageIndex + 1} sent: ${heartbeatData.progress}%`);
 } catch (error) {
...
/**
* Main API handler with heartbeat support
*/
export default async function handler(req, res) {
 ...
 if (req.method === 'GET') {
   return res.status(200).json({ 
     message: 'Podcast Growth Agent API v11 - Production Optimized with Heartbeat',
     status: 'ready',
     features: [
       'Search for SPECIFIC EPISODE by title',
       'Real Whisper transcription',
       'Production-grade response optimization',
       'Heartbeat system prevents timeouts',
 ...
 if (req.method === 'POST') {
   ...
   try {
     ...
     const initialResponse = {
       status: 'processing',
       progress: 10,
       message: "🚀 Starting deep analysis of your episode...",
       episode_title: episode.title_original,
       duration: episode.audio_length_sec,
       note: "Hang tight! We're about to extract insights that'll supercharge your growth! 💪"
     };
     
     res.write(JSON.stringify(initialResponse) + '\n');

     // Set up heartbeat timers for long episodes
     if (episode.audio_length_sec > 900) { // 15+ minutes
       CONFIG.HEARTBEAT.MESSAGES.forEach((_, index) => {
         const timer = setTimeout(() => {
           sendHeartbeat(res, index, episode.title_original);
         }, CONFIG.HEARTBEAT.INTERVAL * (index + 1));
     ...
     // Clear heartbeat timers on error
     heartbeatTimers.forEach(timer => clearTimeout(timer));
